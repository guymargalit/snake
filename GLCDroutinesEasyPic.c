 ...
 * Author of last change: Connor Kerry
 ...
 *     Parameters:      None
 *     Description:		This function reads a byte from the GLCD by reading
 *                      PORTD and cycling the enable pin. To read data from the 
 *                      GLCD, RS must be logic HIGH and RW must be logic HIGH.
 *
 ******************************************************************************/
unsigned char ReadData(){
    unsigned char mydata;
    GLCD_RS_LAT = 1;      // RS must be logic HIGH with RW logic HIGH to tell the
    GLCD_RW_LAT = 1;      // GLCD that data is being read from it.
    GLCD_DATA_TRIS = 0xFF;//set trisd as all inputs
    
    Enable();
    //Delay1TCY();Delay1TCY();Delay1TCY();Delay1TCY(); // Delay 1 us
    mydata = GLCD_DATA_PORT; // Set the Data PORT line to the data input.
    GLCD_DATA_TRIS = 0;
    return mydata;
}
